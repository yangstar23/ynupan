...
window.onload = function () {
    ...
    var noshares = document.getElementsByClassName("noshare");
    ...
    //取消分享
    for (var i = 0; i < noshares.length; i++) {
        noshares[i].onclick = function () {
            var id = this.lastElementChild.value
            var sharenode = this;
            $.ajax({
                type: "post",
                url: "/files/setshare",    //请求发送到dataActiont处
                data: {"id":id,"isshare":0},
                success: function (data) {
                    if (data===1){
                        sharenode.parentElement.parentElement.parentElement.style.display = "none"
                    }else{
                        layer.msg("取消分享异常");
                    }
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (chooseones.length===ids.length){
                    window.location.href = "/files/findallshares";
                }
            }
        })
    };
